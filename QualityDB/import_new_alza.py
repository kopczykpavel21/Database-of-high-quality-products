...
Each processed row also gets a dated row in snapshots.db (via scraper.snapshots),
so price/rating/recommend-rate history accumulates for Alza products too.
...
def upsert_all(df: pd.DataFrame, label: str, conn: sqlite3.Connection):
    ...
    for _, row in qualified.iterrows():
        ...
        # Record today's price/rating/recommend-rate snapshot for history tracking.
        record_snapshot(conn, url, "alza", {
            "Price_CZK":         vals[1],
            "Price_EUR":         None,
            "AvgStarRating":     vals[2],
            "ReviewsCount":      vals[4],
            "RecommendRate_pct": vals[5],
        }, country="CZ")
