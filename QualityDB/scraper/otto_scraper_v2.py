...
def parse_eur(value):
    """Parse a European price string into a float in euros.

    Otto.de embeds prices as strings in __NEXT_DATA__ / JSON-LD:
      • "62,90"      → 62.90   (German decimal, comma separator)
      • "1.299,00"   → 1299.00 (German with thousands dot)
      • "62.90"      → 62.90   (dot-decimal from some API paths)
      • "1299"       → 1299.0  (integer euros)
    The old implementation blindly stripped ALL dots, turning "62.90" → 6290.
    """
    if value is None:
        return None
    ...
    if has_dot and has_comma:
        # German "1.299,00" — comma is decimal, dot is thousands
        if text.rfind(",") > text.rfind("."):
            text = text.replace(".", "").replace(",", ".")
        else:
            # US "1,299.00" — dot is decimal, comma is thousands
            text = text.replace(",", "")
    elif has_comma:
        # German decimal only: "62,90"
        text = text.replace(",", ".")
    elif has_dot:
        # Could be "62.90" (dot=decimal) or "1.000" (dot=thousands)
        parts = text.split(".")
        if len(parts) == 2 and len(parts[1]) <= 2:
            pass  # dot is decimal separator — keep as-is
        else:
            text = text.replace(".", "")  # dot is thousands separator
    try:
