...
def _build_registry() -> list:
    ...
    # ── Weekly — alza.cz live refresh (fetches fresh prices/ratings from Alza.cz) ─
    # Runs Sunday night; the daily snapshot below then captures the fresh data.
    try:
        from scraper.alza_live_scraper import run_alza_live_scraper
        add("Alza.cz live refresh", "CZ", "weekly", run_alza_live_scraper, days=[6])
    except ImportError as e:
        log.warning(f"Alza.cz live refresh not available: {e}")

    # ── Daily — alza.cz snapshot (reads products.db after live refresh) ──────────
    try:
        from scraper.alza_snapshot import run_alza_snapshot
        add("Alza.cz snapshots", "CZ", "daily", run_alza_snapshot)
    except ImportError as e:
        log.warning(f"Alza.cz snapshots not available: {e}")
...
def main() -> None:
    ...
    while True:
        ...
        # ── Post-scrape normalization — categorize any new products scraped tonight
        try:
            import sys as _sys, os as _os
            _srv = _os.path.join(_os.path.dirname(__file__), "..", "server.py")
            _srv = _os.path.normpath(_srv)
            if _os.path.dirname(_srv) not in _sys.path:
                _sys.path.insert(0, _os.path.dirname(_srv))
            from server import _post_scrape_normalize
            _nn = _post_scrape_normalize()
            if _nn:
                log.info(f"[normalize] post-scrape: normalized {_nn} new products")
        except Exception as exc:
            log.warning(f"Post-scrape normalization failed: {exc}")

        # ── Daily og:image fill — fetch og:image for up to 500 products/night
        try:
            import sys as _sys, os as _os
            _srv = _os.path.join(_os.path.dirname(__file__), "..", "server.py")
            _srv = _os.path.normpath(_srv)
            if _os.path.dirname(_srv) not in _sys.path:
                _sys.path.insert(0, _os.path.dirname(_srv))
            from server import _fetch_og_images
            _n = _fetch_og_images(batch=5000)
            log.info(f"[og-images] nightly run: {_n} products processed")
        except Exception as exc:
            log.warning(f"og:image nightly run failed: {exc}")
