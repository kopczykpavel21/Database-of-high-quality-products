...
def parse_eur(value):
    ...
    Otto.de prices come as strings like "62,90", "1.299,00", or "62.90".
    ...
    if has_dot and has_comma:
        if text.rfind(",") > text.rfind("."):
            text = text.replace(".", "").replace(",", ".")   # German "1.299,00"
        else:
            text = text.replace(",", "")                     # US "1,299.00"
    elif has_comma:
        text = text.replace(",", ".")  # German "62,90"
    elif has_dot:
        parts = text.split(".")
        if len(parts) == 2 and len(parts[1]) <= 2:
            pass  # dot is decimal "62.90" — keep as-is
        else:
            text = text.replace(".", "")  # dot is thousands "1.000"
    try:
